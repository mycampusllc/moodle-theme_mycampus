...
function theme_mycampus_process_css($css, $theme) {

    // Set the background image for the logo.
    $logo = $theme->setting_file_url('logo', 'logo');
    $css = theme_mycampus_set_logo($css, $logo);
    ...
    $css = theme_mycampus_set_customcss($css, $customcss);
 ...
 */
function theme_mycampus_set_logo($css, $logo) {
    $tag = '[[setting:logo]]';
    $replacement = $logo;
 ...
 * @return bool
 */
function theme_mycampus_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    if ($context->contextlevel == CONTEXT_SYSTEM and $filearea === 'logo') {
        $theme = theme_config::load('mycampus');
 ...
 */
function theme_mycampus_set_customcss($css, $customcss) {
 ...
 *
 * Do not add mycampus specific logic in here, child themes should be able to
 ...
 */
function theme_mycampus_get_html_for_settings(renderer_base $output, moodle_page $page) {
    global $CFG;
    $return = new stdClass;
...
/**
 * All theme functions should start with theme_mycampus_
 * @deprecated since 2.5.1
 */
function mycampus_process_css() {
 ...
 * @deprecated since 2.5.1
 */
function mycampus_set_customcss() {
    throw new coding_exception('Please call theme_'.__FUNCTION__.' instead of '.__FUNCTION__);
}
...
//处理端口不为3306的情况
$theme_mycampus_dboptions = $CFG->dboptions;
if($theme_mycampus_dboptions['dbport']!="")		{
	$dbA->Connect($CFG->dbhost.":".$theme_mycampus_dboptions['dbport'],$CFG->dbuser,$CFG->dbpass,$CFG->dbname);
}
else	{
...
//mycampus_GetCourseTeacherNumber
function mycampus_GetCourseTeacherNumber($id)		{
	global $dbA,$adodbcacheseconds,$indexshowcoursenumber;
...
//mycampus_GetCourseQuestionNumber
function mycampus_GetCourseQuestionNumber($id)		{
	global $dbA,$adodbcacheseconds,$indexshowcoursenumber;
...
function mycampus_GetCourseEnrollmentStudentNumber($id)		{
	global $dbA,$adodbcacheseconds,$indexshowcoursenumber;
...
function mycampus_GetCourseSectionNumber($id)		{
	global $dbA,$adodbcacheseconds,$indexshowcoursenumber;
...
function mycampus_GetCourseQuizNumber($id)		{
	global $dbA,$adodbcacheseconds,$indexshowcoursenumber;
...
function mycampus_GetCourseOneMouthActivesNumber($id)		{
	global $dbA,$adodbcacheseconds,$indexshowcoursenumber;
...
function mycampus_substr_for_utf8($sourcestr,$cutlength)
{
	$returnstr="";
	$i=0;
	$n=0;
...
function mycampus_objectToArray($rs_a)				{
...
function mycampus_randpictice($id,$course='course')
{

	$CourseFile = "theme/mycampus/images/".$course."_".$id.".jpg";
	if(is_file($CourseFile))			{
		$returnCourseGroupImg = $CourseFile;
	}
	else	{
		$returnCourseGroupImg = "/theme/mycampus/images/".$course."_0000.jpg";
	}

	return $returnCourseGroupImg;
}

function mycampus_indexcoursedetailshow($html)		{
	global $dbA,$adodbcacheseconds,$indexshowcoursenumber;
	...
	for($i=0;$i<$courseNUM;$i++)
	{
		...
		//echo $summary."<br>";
		$returnCourseGroupImg = mycampus_randpictice($id);
		//得到老师信息
		$mycampus_GetCourseTeacherNumber = mycampus_GetCourseTeacherNumber($id);
		mycampus__ReturnTableCourseYindaoye($html,$fullname,$summary,$returnCourseGroupImg,$id,$mycampus_GetCourseTeacherNumber,$shortname);
	}
	print "</td></tr></table>";
}
...
//返回课程引导页输出
function mycampus__ReturnTableCourseYindaoye($html,$fullname,$summary,$returnCourseGroupImg,$id,$mycampus_GetCourseTeacherNumber,$shortname)
{
	global $CFG,$dbA;
	global $dbA,$adodbcacheseconds,$indexshowcoursenumber;
	$indexshowcoursenumber	= $html->indexshowcoursenumber;
	...
	$mycampus_GetCourseTeacherNumber_TEXT = join(',',$mycampus_GetCourseTeacherNumber);
	print "<link rel=\"stylesheet\" type=\"text/css\" href=\"/theme/mycampus/style/style_4.css\" />";
	print "
				<script type=\"text/javascript\" src=\"/theme/mycampus/js/jquery-1.7.1.min.js\"></script>
				<script src=\"/theme/mycampus/js/highcharts/highcharts.js\"></script>
				<script src=\"/theme/mycampus/js/highcharts/modules/exporting.js\"></script>
				<style type=\"text/css\">
				${demo.css}
				</style>
				";
	...
	print "<tr><td width=15>&nbsp;</td><td><BR>
		$shortname [教师:".$mycampus_GetCourseTeacherNumber_TEXT."]
		&nbsp;&nbsp;
...
function mycampus_IndexMainCourseShow($html)			{
	global $dbA,$adodbcacheseconds,$indexshowcoursenumber;
	$indexshowcoursenumber	= $html->indexshowcoursenumber;
	$adodbcacheseconds		= $html->adodbcacheseconds;
	print "<link rel=\"stylesheet\" type=\"text/css\" href=\"/theme/mycampus/style/style_".$indexshowcoursenumber.".css\" />";
	//print_R($html);exit;
	if($html->isshowlogo)				{
		$logoArray = explode('/',$html->logo);
		$filename  = array_pop($logoArray);
		$sql			= "select * from mdl_files where component='theme_mycampus' and filearea='logo' and filename='$filename'";
		//echo $sql."<br>";
		$rs				= $dbA->CacheExecute($adodbcacheseconds,$sql);
		$rs_a			= $rs->GetArray();
		...
		if($contenthash!="")			{
			//pluginfile.php/1/theme_mycampus/logo/0/logo.jpg
			$URL = "pluginfile.php/".$contextid."/".$component."/".$filearea."/".$itemid."/".$filename."";
		}
		else	{
			$URL = "/theme/mycampus/pix/logo.jpg";
		}
	...
	$coursestatistics_teachername			= get_string('coursestatistics_teachername', 'theme_mycampus');
	$coursestatistics_users					= get_string('coursestatistics_users', 'theme_mycampus');
	$coursestatistics_sections				= get_string('coursestatistics_sections', 'theme_mycampus');
	$coursestatistics_quizs					= get_string('coursestatistics_quizs', 'theme_mycampus');
	$coursestatistics_questions				= get_string('coursestatistics_questions', 'theme_mycampus');
	$coursestatistics_actives				= get_string('coursestatistics_actives', 'theme_mycampus');
	$wonderfulcourse						= get_string('wonderfulcourse', 'theme_mycampus');
	$wonderfulcoursedesc					= get_string('wonderfulcoursedesc', 'theme_mycampus');
	$hotcourse								= get_string('hotcourse', 'theme_mycampus');
	$hotcoursedesc							= get_string('hotcoursedesc', 'theme_mycampus');
	...
	if($html->wonderfulcourse&&$html->wonderfulcoursenumber!="")				{
		...
		for($i=0;$i<sizeof($CourseInforArray_ARRAY['id']);$i++)
		{
			...
			$mycampus_GetCourseEnrollmentStudentNumber	= mycampus_GetCourseEnrollmentStudentNumber($id);
			$mycampus_GetCourseQuestionNumber			= mycampus_GetCourseQuestionNumber($id);
			$mycampus_GetCourseQuizNumber				= mycampus_GetCourseQuizNumber($id);
			$mycampus_GetCourseSectionNumber				= mycampus_GetCourseSectionNumber($id);
			$mycampus_GetCourseOneMouthActivesNumber		= mycampus_GetCourseOneMouthActivesNumber($id);
			$mycampus_GetCourseTeacherNumber				= mycampus_GetCourseTeacherNumber($id);

			$CourseNameShowText					= mycampus_substr_for_utf8($fullname,10);

			$returnCourseGroupImg				= mycampus_randpictice($id);
			$USER_OJB = $_SESSION['USER'];
			if($USER_OJB->username=='admin')		{
				$indexpage_uploadimagebuttontitle	= get_string("indexpage_uploadimagebuttontitle","theme_mycampus");
				$indexpage_uploadimagebuttonname	= get_string("indexpage_uploadimagebuttonname","theme_mycampus");
				...
				<li>
				  <div class=\"dandian_info\"> <img src=\"$returnCourseGroupImg\" />
					<div class=\"dandian_text\">
					  <p>".$coursestatistics_teachername.": ".join(',',$mycampus_GetCourseTeacherNumber)."</p>
					  <p>".$coursestatistics_users.": $mycampus_GetCourseEnrollmentStudentNumber</p>
					  <p>".$coursestatistics_sections.": $mycampus_GetCourseSectionNumber</p>
					  <p>".$coursestatistics_quizs.": $mycampus_GetCourseQuizNumber</p>
					  <p>".$coursestatistics_questions.": $mycampus_GetCourseQuestionNumber</p>
					  <p>".$coursestatistics_actives.": $mycampus_GetCourseOneMouthActivesNumber</p>
					</div>
				  </div>
				  <BR>
	...
	if($html->hotcourse)				{
		...
		for($i=0;$i<sizeof($rs_a);$i++)		{
			if($rs_a[$i]['courseid']>1)		{
				...
				$mycampus_GetCourseOneMouthActivesNumberALL[$courseid] = $rs_a[$i]['num'];
			}
		}
		...
		for($i=0;$i<sizeof($CourseInforArray_ARRAY['id']);$i++)
		{
			...
			$mycampus_GetCourseQuizNumber				= mycampus_GetCourseQuizNumber($id);
			$mycampus_GetCourseSectionNumber				= mycampus_GetCourseSectionNumber($id);
			$mycampus_GetCourseOneMouthActivesNumber		= $mycampus_GetCourseOneMouthActivesNumberALL[$id];
			$mycampus_GetCourseTeacherNumber				= mycampus_GetCourseTeacherNumber($id);
	...
	if($html->day30userinfor)										{
		...
		$indexpage_statistics_name				= get_string('indexpage_statistics_name', 'theme_mycampus');
		$day30userinfor							= get_string('day30userinfor', 'theme_mycampus');
		$day30userinforunit						= get_string('day30userinforunit', 'theme_mycampus');
	...
	if($html->day30accessinfor)				{
		...
		$day30accessinfor							= get_string('day30accessinfor', 'theme_mycampus');
		$day30accessinforunit						= get_string('day30accessinforunit', 'theme_mycampus');
...
if($action=="imagesetting"&&$id!="")			{
	if($coursetroup=="group")			{
		$NewCourseImage = "theme/mycampus/images/group_".$id.".jpg";
	}
	else	{
		$NewCourseImage = "theme/mycampus/images/course_".$id.".jpg";
	}
	...
	$GOON		= 1;
	if(!is_file($filename))		{
		$indexpage_uploadimagefailed			= get_string('indexpage_uploadimagefailed', 'theme_mycampus');
		...
		elseif($filename_type=="image/png")									{
			...
			$indexpage_uploadimageformaterror			= get_string('indexpage_uploadimageformaterror', 'theme_mycampus');
	...
	if($GOON==1)												{
			...
			$indexpage_uploadimage			= get_string('indexpage_uploadimage', 'theme_mycampus');
			$indexpage_gobacktoindexpage	= get_string('indexpage_gobacktoindexpage', 'theme_mycampus');
...
function mycampus_coursecontextgroup($html)										{
	global $dbA,$adodbcacheseconds,$indexshowcoursenumber;
...
function mycampus_left_button($html)		{
	//print_R($html);
	print "
	<fieldset class=\"coursesearchbox invisiblefieldset\">";
	$coursestatistics = get_string('coursestatistics', 'theme_mycampus');
	...
	$coursecategorystatistics = get_string('coursecategorystatistics', 'theme_mycampus');
	...
	$useraccessstatistics = get_string('useraccessstatistics', 'theme_mycampus');
	...
	$activeteachersstatistics = get_string('activeteachersstatistics', 'theme_mycampus');
	...
	$activestudentsstatistics = get_string('activestudentsstatistics', 'theme_mycampus');
